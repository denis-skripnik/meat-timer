...
- The foreground keeper uses a separate low-importance silent notification channel and does not refresh its notification every few seconds, so it should not make repeated notification sounds.
- If battery restrictions are still active, the app shows TalkBack-readable guidance and a button to open app settings so the user can choose an unrestricted/no-optimization battery mode.
